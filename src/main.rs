...
fn write_image(filename: &str, pixels: &[u8], bounds: (usize, usize)) -> Result<(), std::io::Error> {
    let colour_gradient = colours::gradient::interpolate_colours(
        ...
        256);
    let colour_limit = colours::gradient::RGBColour{ red: 0, green: 0, blue: 0 };
    ...
    for (x, y, pixel) in img_buf.enumerate_pixels_mut() {
        ...
        let colour: &colours::gradient::RGBColour = match pixels[index] {
            0 => &colour_limit,
            _ => &colour_gradient[pixels[index] as usize]
        };
...
fn main() {
    ...
    if args.len() != 5 {
        ...
        "Usage: mandelbrot FILE PIXELS UPPERLEFT LOWERRIIGHT").unwrap();
        std::process::exit(1);
    }
    ...
    writeln!(std::io::stdout(), "Using {} theads.", threads).unwrap();
